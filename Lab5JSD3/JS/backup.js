...
function resetTable() {
    createTable(cars, 'list'); // Загрузка исходных данных в таблицу
    currentTable = cars.slice(); // Обновление переменной currentTable
}
...
let sortTable = (idTable, data) => {
    ...
    rowData.sort((first, second) => {
        for (let i = 0; i < sortArr.length; i++) {
            ...
            if (isNaN(firstValue) || isNaN(secondValue)) {
                // Если значения не являются числами, оставляем сравнение как строки
    ...
    currentTable = rowData.map(row => {
        ...
        for (let i = 0; i < row.cells.length; i++) {
            ...
            let header = table.rows[0].cells[i].innerHTML; // Получаем название столбца из заголовка
...
function resetGraphSettings() {
    // Восстанавливаем значения по умолчанию для элементов формы
...
// Добавляем обработчик события на кнопку сброса графика
let resetGraphButton = document.getElementById('resetGraphButton');
resetGraphButton.addEventListener('click', function() {
    resetGraph();
    resetGraphSettings(); // Сбрасываем настройки графика
});

// Определяем функцию сброса графика
function resetGraph() {
    svg.selectAll('*').remove(); // Очищаем все элементы внутри svg
}
